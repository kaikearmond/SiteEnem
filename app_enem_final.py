...
st.markdown(
    ...
    .kpi-card {{
        ...
        border: 1px solid rgba(217, 226, 241, 0.5);
        min-height: 118px;
    }}
    .kpi-title {{
...
def estilizar_figura(fig: go.Figure, altura: int = 360) -> go.Figure:
    fig.update_layout(
        ...
        hoverlabel=dict(bgcolor='#0F172A', bordercolor='#0F172A', font=dict(color='white', size=13, family='Arial')),
    ...
    fig.update_xaxes(
        showgrid=False,
        zeroline=False,
        tickfont=dict(color=CORES['texto_claro'], size=12),
        title_font=dict(color=CORES['texto_suave'], size=13)
    )
    fig.update_yaxes(
        gridcolor=CORES['linha'],
        zeroline=False,
        tickfont=dict(color=CORES['texto_claro'], size=12),
        title_font=dict(color=CORES['texto_suave'], size=13)
    )
...
with st.sidebar:
    ...
    regiao = st.selectbox('Região', ['TODAS'] + REGIOES, format_func=lambda x: 'Todas as regiões' if x == 'TODAS' else x)
    ...
    uf = st.selectbox('Estado (UF)', ['TODAS'] + ufs_filtradas, format_func=lambda x: 'Todos os estados' if x == 'TODAS' else x)
...
st.markdown(
    ...
    <div class="bloco-topo">
        ...
        <div style="font-size:1rem;opacity:0.92;">Análise visual com foco em distribuição, comparação regional e ranking por UF.</div>
    </div>
    """,
    unsafe_allow_html=True,
)
...
st.markdown(f'<div class="resumo-linha">{contagem}</div>', unsafe_allow_html=True)
...
fig_hist = estilizar_figura(fig_hist, altura=430)
...
fig_box.update_traces(
    ...
    hovertemplate=(
        '<b>Região: %{x}</b><br>'
        'Máx: %{upperfence:.1f}<br>'
        'Q3: %{q3:.1f}<br>'
        'Mediana: %{median:.1f}<br>'
        'Q1: %{q1:.1f}<br>'
        'Mín: %{lowerfence:.1f}<extra></extra>'
    ),
    hoverlabel=dict(
        bgcolor='#0F172A',
        bordercolor='#0F172A',
...
fig_box = estilizar_figura(fig_box, altura=430)

# Ranking UF - versão mais legível
media_uf = (
...
ranking_uf = media_uf.head(12).sort_values('Média', ascending=True).copy()
...
fig_uf = go.Figure(
    go.Bar(
        ...
        y=ranking_uf['UF'],
        orientation='h',
        text=[f'{v:.1f}' for v in ranking_uf['Média']],
        textposition='outside',
        marker=dict(
            color=ranking_uf['Média'],
            colorscale=[[0, '#BFDBFE'], [0.45, '#60A5FA'], [1, '#1D4ED8']],
            line=dict(color='#1E3A8A', width=0.5),
        ),
        customdata=np.stack([ranking_uf['Quantidade']], axis=-1),
...
fig_uf.update_layout(
    ...
    yaxis_title='',
    xaxis=dict(range=[max(0, ranking_uf['Média'].min() - 20), ranking_uf['Média'].max() + 25]),
)
fig_uf.update_traces(textfont=dict(size=13, color=CORES['texto']))
fig_uf = estilizar_figura(fig_uf, altura=520)
fig_uf.update_yaxes(tickfont=dict(size=13, color=CORES['texto_suave']))
fig_uf.update_xaxes(showgrid=True, gridcolor=CORES['linha'])
...
fig_faixas = estilizar_figura(fig_faixas, altura=430)

# Tabela resumo em padrão visual mais consistente
ranking_tabela = media_uf.head(10).copy()
ranking_tabela['Média'] = ranking_tabela['Média'].round(1)
ranking_tabela['Quantidade'] = ranking_tabela['Quantidade'].map(lambda x: f"{x:,.0f}".replace(',', '.'))
ranking_tabela['Posição'] = [f'{i}º' for i in range(1, len(ranking_tabela) + 1)]
ranking_tabela = ranking_tabela[['Posição', 'UF', 'Média', 'Quantidade']]

fig_tabela = go.Figure(
    data=[go.Table(
        columnwidth=[0.7, 0.7, 1.0, 1.1],
        header=dict(
            values=['<b>Posição</b>', '<b>UF</b>', '<b>Média</b>', '<b>Quantidade</b>'],
        ...
        cells=dict(
            values=[ranking_tabela[c] for c in ranking_tabela.columns],
            fill_color=[['#F8FBFF', '#EEF4FF'] * 5],
            align=['center', 'center', 'center', 'center'],
            font=dict(color=CORES['texto'], size=12),
            height=34,
            line_color='rgba(217, 226, 241, 0.9)'
        )
    )]
)
...
col_c, col_d = st.columns([1.15, 0.85])
with col_c:
    st.markdown('<div class="secao-card"><div class="secao-titulo">Ranking das UFs</div><div class="secao-subtitulo">Estados ordenados pela média da nota escolhida no filtro lateral, com visual mais limpo e legível.</div>', unsafe_allow_html=True)
...
st.markdown('<div class="secao-card"><div class="secao-titulo">Resumo dos melhores desempenhos</div><div class="secao-subtitulo">Painel consolidado com os estados de maior média e volume de participantes, em um visual alinhado ao restante do dashboard.</div>', unsafe_allow_html=True)
